use serde::{Serialize, Deserialize};
use uuid::Uuid;

use crate::domain::entities::author::Author;
use crate::domain::entities::publisher::Publisher;
use crate::domain::entities::gender::Gender;


#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
pub struct Book {
    id: Uuid,
    title: String,
    subtitle: Option<String>,
    authors: Vec<Author>,
    publisher: Option<Publisher>,
    series_collection: Option<i32>,
    volume: Option<i32>,
    edition: Option<i32>,
    publication_year: Option<i32>,
    pages: Option<i32>,
    language: Option<String>,
    isbn: String,
    genders: Vec<Gender>,
    synopsis: Option<String>,
    cover: Option<String>
}

impl Book {
    pub fn new(
        id: Uuid,
        title: String,
        subtitle: Option<String>,
        authors: Vec<Author>,
        publisher: Option<Publisher>,
        series_collection: Option<i32>,
        volume: Option<i32>,
        edition: Option<i32>,
        publication_year: Option<i32>,
        pages: Option<i32>,
        language: Option<String>,
        isbn: String,
        genders: Vec<Gender>,
        synopsis: Option<String>,
        cover: Option<String>
    ) -> Book {
        Book {
            id,
            title,
            subtitle,
            authors,
            publisher,
            series_collection,
            volume,
            edition,
            publication_year,
            pages,
            language,
            isbn,
            genders,
            synopsis,
            cover
        }
    }

    pub fn get_id(&self) -> Uuid {
        self.id
    }

    pub fn get_title(&self) -> String {
        self.title.clone()
    }

    pub fn get_subtitle(&self) -> Option<String> {
        self.subtitle.clone()
    }

    pub fn get_authors(&self) -> Vec<Author> {
        self.authors.clone()
    }

    pub fn get_publisher(&self) -> Option<Publisher> {
        self.publisher.clone()
    }

    pub fn get_series_collection(&self) -> Option<i32> {
        self.series_collection.clone()
    }

    pub fn get_volume(&self) -> Option<i32> {
        self.volume.clone()
    }

    pub fn get_edition(&self) -> Option<i32> {
        self.edition.clone()
    }

    pub fn get_publication_year(&self) -> Option<i32> {
        self.publication_year.clone()
    }

    pub fn get_pages(&self) -> Option<i32> {
        self.pages.clone()
    }

    pub fn get_language(&self) -> Option<String> {
        self.language.clone()
    }

    pub fn get_isbn(&self) -> String {
        self.isbn.clone()
    }

    pub fn get_genders(&self) -> Vec<Gender> {
        self.genders.clone()
    }

    pub fn get_synopsis(&self) -> Option<String> {
        self.synopsis.clone()
    }

    pub fn get_cover(&self) -> Option<String> {
        self.cover.clone()
    }

    pub fn set_title(&mut self, new_title: String) {
        self.title = new_title;
    }

    pub fn set_subtitle(&mut self, new_subtitle: Option<String>) {
        self.subtitle = new_subtitle;
    }

    pub fn set_authors(&mut self, new_authors: Vec<Author>) {
        self.authors = new_authors;
    }

    pub fn set_publisher(&mut self, new_publisher: Option<Publisher>) {
        self.publisher = new_publisher;
    }

    pub fn set_series_collection(&mut self, new_series_collection: Option<i32>) {
        self.series_collection = new_series_collection;
    }

    pub fn set_volume(&mut self, new_volume: Option<i32>) {
        self.volume = new_volume;
    }

    pub fn set_edition(&mut self, new_edition: Option<i32>) {
        self.edition = new_edition;
    }

    pub fn set_publication_year(&mut self, new_publication_year: Option<i32>) {
        self.publication_year = new_publication_year;
    }

    pub fn set_pages(&mut self, new_pages: Option<i32>) {
        self.pages = new_pages;
    }

    pub fn set_language(&mut self, new_language: Option<String>) {
        self.language = new_language;
    }

    pub fn set_isbn(&mut self, new_isbn: String) {
        self.isbn = new_isbn;
    }

    pub fn set_genders(&mut self, new_gender: Vec<Gender>) {
        self.genders = new_gender;
    }

    pub fn set_synopsis(&mut self, new_synopsis: Option<String>) {
        self.synopsis = new_synopsis;
    }

    pub fn set_cover(&mut self, new_cover: Option<String>) {
        self.cover = new_cover;
    }
}